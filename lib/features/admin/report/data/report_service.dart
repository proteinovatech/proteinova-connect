...
Future<Map<String, dynamic>> getWarehouseReport({
  ...
  final list =
      (response.data['dispatches'] as List<dynamic>?) ?? [];
...
for (var item in list) {
  ...
  final eggs =
      (item["total_eggs"] as num?)?.toInt() ?? 0;

  dayWiseDispatch[day] =
      (dayWiseDispatch[day] ?? 0) + eggs;
}
List<Map<String, dynamic>> volumeChart =
    dayWiseDispatch.entries.map((e) {
  return {
    "day": e.key,
    "dispatched": e.value,
  };
  ...
  final availableStock =
      response.data['availableStock'] ?? 0;
      int totalTrays = 0;
...
double onTimeDelivery =
    list.isEmpty ? 0 : (deliveredCount / list.length) * 100;
  ...
  for (var item in list) {
    int units =
    int.tryParse(item['total_eggs'].toString()) ?? 0;
    String dest =
        item['destination']?.toString() ?? "Unknown";

    destMap[dest] = (destMap[dest] ?? 0) + units;
  }

  List<Map<String, dynamic>> destinations =
      destMap.entries.map((e) {
    return {
      "name": e.key,
      "trays": e.value,
      "progress":
          totalEggs > 0 ? e.value / totalEggs : 0,
    };
  }).toList();
  ...
  return {
    "stats": [
      ...
      },
      {
        ...
        "amount":  NumberFormat.decimalPattern('en_IN')
      .format(totalEggs),
        "growth": "",
        "icon": "truck",
        "color": "green",
